...
import com.example.fistproject.entity.Comment;
...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        ...
        /*Case 3: 9번 게시글의 모든 댓글 조회*/
        {
            ...
            List<Comment> comments = commentRepository.findByArticleId(articleId);
            // 3. 예상 데이터
            Article article = null; // 9번 글은 존재하지 않음
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "9번 글은 존재하지 않으므로 댓글 또한 존재 X");
        }
        /*Case 4: 999번 게시글의 모든 댓글 조회*/
        {
            ...
            List<Comment> comments = commentRepository.findByArticleId(articleId);
            // 3. 예상 데이터
            Article article = null; // 999번 글은 존재하지 않음
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "999번 글은 존재하지 않으므로 댓글 또한 존재 X");
        }
        /*Case 5: -1번 게시글의 모든 댓글 조회*/
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "-1번 글은 존재하지 않으므로 댓글 또한 존재 X");
        }
    }
